...
// 遞迴渲染目錄樹（每個目錄有容量按鈕）
function renderTree(dir: Directory, parentElement: HTMLElement) {
    ...
    dir.children.forEach((c: FileSystemItem) => {
...
if (typeof window !== "undefined") {
    ...
    document.getElementById("searchBtn")!.onclick = () => {
        // 取得輸入值並移除開頭的點號（如果有）
        let ext = (document.getElementById("searchExt") as HTMLInputElement).value.trim();
